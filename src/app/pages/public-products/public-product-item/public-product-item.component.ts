...
export class PublicProductItemComponent implements OnInit {
 ...
 // apply grid-img  class to image only whn it is shown in grid
 //else in product detail page show full size image
  isGrid = true;
  ...
  ngAfterViewInit(): void {
    this.isGrid = !(!this.buttons[0] && !this.buttons[1] && !this.buttons[2]);
