  ...
  //history$
   ...
   // tap(val=> console.log('snaps', val)),
    ...
    allProductForView$ = combineLatest([
      ...
      //first(), // we don't want observable to continuously run
      map(([products, makes, types,discounts]) => this.mapProductsForView(products, makes,types,discounts)),

    );

    mapProductsForView(products: Product[], makes: SelectValues[], types: SelectValues[], discounts?: any): Product[] {
      return products.map(product =>  this.mapProductForView(product, makes, types, discounts));
    }
    mapProductForView(product: Product, makes: SelectValues[], types: SelectValues[], discounts?: any): Product {
     // console.log('mapProductForView Data',product);
        ...
        let discountDesc = '';
        if(discounts){
          discountValue = discounts.find(c => product.discountId === c.id)?.value || 0;
          discountDesc = discounts.find(c => product.discountId === c.id)?.desc || '';
        }
        return ({
        ...product,
        makeView,
   ...
   addProduct(
    ...
    if (this.user) {
     // this.uiService.presentLoading('Submitting Complaint...');
      const newProduct = {name,desc, type,subType,make,price,discountId,photoURL,count, createdOn: new Date(), createdBy: this.user.uid}
